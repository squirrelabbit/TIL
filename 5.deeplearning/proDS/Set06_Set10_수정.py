...
var_list=['price', 'bedrooms', 'bathrooms', 
...
# 가장 큰 값을 가지고 있던 모델(그룹 1)과 가장 낮은 값을 가지고 있던 모델(그룹 2)
# 간의 운행 일수 대비 운행거리 평균이 다른지
# 적절한 통계 검정을 수행 => 두 집단 간의 평균 차이 검정 수행
# 등분산을 가정=> 독립인 2표본 t 검정
